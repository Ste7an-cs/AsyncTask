 ...
 *
 * suspend_until 委托基类阻塞（cv 等待，不空转）；首次 suspend_until 用 std::call_once
 * 创建一个绑定当前线程的常驻“事件协程”持续分发 Qt 事件。退出流程：Coro::quit() 调
 * signalExit() 设全局退出标志，各线程的泵协程醒来后自行退出，从而 ~scheduler 无需等待
 * 无限协程即可干净析构。
 */
...
protected:
    ...
    std::once_flag pump_once_;               ///< 每实例一次（每个线程有自己的调度器实例）
