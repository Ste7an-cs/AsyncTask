...
namespace {
///
/// \brief drainUntilIdle 关机排空：应用仍存活时反复驱动 Qt 事件与主线程协程调度，
///     让被 aboutToQuit 唤醒的协程跑完并处理其投递的 deleteLater。
///     否则残留的挂起协程会在 QCoreApplication 析构后被 boost.fiber 收尾流程唤醒，
///     届时访问已销毁的事件系统而崩溃，或因协程未终结导致线程无法退出而卡死。
