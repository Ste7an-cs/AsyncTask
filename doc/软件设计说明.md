...
- **DD-6 主循环模型与安全退出（满足 RX_QT_INTEGRATION、RX_DESIGN_MAINLOOP）**：以 `Coro::exec()`（`block.wait`）为主循环；Qt 事件由各线程的**常驻泵协程**（每 Qt 线程一个，首次 `suspend_until` 用 `std::call_once` 懒启动、终身复用）在 worker 协程上下文里持续分发，从而 Qt 回调可安全做协程阻塞且不阻塞线程。`Coro::quit()` 依次：广播 `aboutToQuit` → **在泵仍存活时** `drainUntilIdle()` 排空在途协程与事件 → 设全局退出标志令泵协程自行终止 → 关闭并 **join** 工作线程池 → `QCoreApplication::exit()`。排空须先于停泵（被唤醒的工作线程协程要靠其线程的泵继续跑到终止）；停池须 join 到工作线程真正退出（否则静态析构会先销毁 `FiberGlobalQueue`，而尚未退出的工作线程仍在 `pick_next()` 访问它 → UAF，详见 §5.3）。**依赖性**：不得以 `QCoreApplication::exec()` 驱动（同线程互斥）；Qt 相关能力依赖宿主工程 `QT` 配置（core/network）启用。
...
**图例说明**：**建立阶段**：`coro(sock).readAll()` 建 Awaitable，把 `readyRead` 连接为“读取并 push 数据块”、把 `disconnected/destroyed` 连接为“关闭 channel”；若建连时套接字已有缓冲数据则**立即 push**，规避“先检查后等待”竞态导致的首包丢失。**消费阶段**：socket 方法返回的 `shared_ptr<Awaitable>` 由 Qt 回调和 `generate` 强持有，循环 `next()→pop()`——无数据则让出，`readyRead` 到来则产出一个 `QByteArray`。**终止阶段**：正常 socket 关闭给出 `no_message` 并让迭代自然结束；socket/local socket/SSL 错误为 typed error。所有 QObject 操作均在对象所属线程执行，跨线程动作使用 queued invocation。
...
sequenceDiagram
    ...
    Note over Sch: 泵仍存活，循环驱动被唤醒协程跑到终止/事件跑净
    App->>Sch: signalExit() 置全局退出标志
    Sch-->>Sch: 各线程常驻泵协程醒来看到标志 → 自行终止
    App->>Pool: block.close() → FibersPool.close(): 唤醒并 join 各工作线程
    Note over Pool: join 到工作线程真正退出，其后不再访问 FiberGlobalQueue
    App->>App: QCoreApplication::exit()
    Note over App: 协程、泵协程、工作线程均已收敛，避免退出崩溃/卡死
```

**图例说明**：解决“协程/泵协程/工作线程活过 Qt 应用对象或全局单例、退出期访问已销毁资源导致崩溃/卡死”的问题。**① 广播**：主动触发 `aboutToQuit`，各 `Awaitable` 借此关闭自己的 channel，挂在 `await()` 的协程被唤醒返回、就地收尾。**② 排空（泵仍存活）**：`drainUntilIdle()` 循环执行 `processEvents`、派发 `deleteLater`、`yield`，直到在途协程与事件跑净——此步必须在停泵之前，被唤醒的工作线程协程要靠其所在线程的泵继续驱动事件与调度才能跑到终止。**③ 停泵**：`signalExit()` 置全局退出标志，各线程常驻泵协程醒来看到标志即自行终止，从而 `~scheduler` 不会因等待无限协程而挂死。**④ 停池并 join**：`FibersPool::close()` 先 `block.close()` 唤醒各工作线程，再 **join** 到它们从 `worker()` 真正返回；这一步是关键——`FibersPool` 与 `FiberGlobalQueue` 均为 Meyers 单例且前者先构造、后者后构造，进程静态析构按逆序会先销毁 `FiberGlobalQueue`，若不 join 就返回，尚未退出的工作线程会在 `pick_next()` 里访问已销毁的就绪集合而 UAF 崩溃（详见 §5.3）。**⑤ 停机**：`QCoreApplication::exit()`。
...
stateDiagram-v2
    ...
    活动 --> 有数据: 来源投递数据
    有数据 --> 活动: await 取走数据
    活动 --> Channel关闭: 来源析构或应用退出
    有数据 --> Channel关闭: 关闭后可继续取空余量
    活动 --> 已关闭并清理: 显式 close
    有数据 --> 已关闭并清理: 显式 close
    Channel关闭 --> 已关闭并清理: 显式 close
    活动 --> 已析构: 最终析构并清理
    有数据 --> 已析构: 最终析构并清理
    Channel关闭 --> 已析构: 最终析构并清理
    已关闭并清理 --> 已析构: 最终析构（不重复清理）
    已析构 --> [*]
```
...
- **工作线程**：`FibersPool`（CSC_EXECUTOR）首次使用时创建 N=`max(硬件并发,2)` 个工作线程，各安装 `QtFiberScheduler` 并 `block.wait()` 挂起（线程不退出但可调度协程）；`FibersPool::close()` 唤醒各线程并 **join** 到它们真正退出，确保工作线程不晚于全局单例（`FiberGlobalQueue`）存活。
...
- **Awaitable/channel**：信号/future 等 `coro()` 工厂按值返回 move-only `Awaitable`；socket 包装器方法返回 `shared_ptr<Awaitable<T>>`。来源析构或 `aboutToQuit` 时关闭 channel；首次显式 `close` 或最终析构中先发生的一方通过 `AwaitableCloseGuard` 恰好一次执行 `setOnClose` 注入的断连清理。
- **退出**：`Coro::quit()`（CSC_TASK）时序见 §4.2.7——广播 `aboutToQuit` → `drainUntilIdle` 排空（泵仍存活）→ `signalExit()` 停泵 → 关线程池并 join 工作线程 → `exit()`。
...
**单元设计决策**：`FiberThreadBlock` 承担“阻止线程退出但允许协程调度”的通用基元，退出路径统一由 `wait()` 内部处理（调用方无需逐一处理内部资源收尾）。`FibersPool::close()` 采用“关闭 + join”而非仅“关闭”：关闭线程阻塞唤醒工作线程后，必须 join 到它们从 `worker()` 真正返回，才能保证退出流程返回前工作线程已全部停止。

**设计约束**：`FibersPool` 为进程内单例；工作线程数默认 `max(硬件并发,2)`。
- **析构顺序安全（对应 RX_QT_INTEGRATION 安全退出）**：`FibersPool` 与就绪协程集合 `FiberGlobalQueue`（CSU_SCHEDULER）均为 Meyers 单例（函数内静态）。`FibersPool` 在 `FiberApplication` 构造时即被构造，`FiberGlobalQueue` 在首次调度（`awakened`/`pick_next`）时才惰性构造，故 `FibersPool` 先构造、`FiberGlobalQueue` 后构造；C++ 规定局部静态按构造逆序析构，于是进程退出时 `FiberGlobalQueue` 先于 `~FibersPool` 被销毁。工作线程在 `worker()` 返回前，其调度器仍会在 `pick_next()` 中访问 `FiberGlobalQueue`。若 `close()` 仅 `block.close()` 唤醒而不 join，`Coro::quit()` 会立即返回、主流程跑到 `main()` 结束触发静态析构，此时尚未退出的工作线程访问已销毁的 `FiberGlobalQueue` → use-after-free 崩溃。因此 `close()` 必须 join 至全部工作线程退出，把工作线程生命周期收敛在两个单例析构之前。`~FibersPool` 中的 join 保留 `joinable()` 判断，与 `close()` 的 join 幂等。
...
- **FibersPool（工作线程池，单例）**：字段 `vector<thread> tds_`；`FiberThreadBlock block_`。`instance()`：首次调用创建 N=`max(硬件并发,2)` 个工作线程，每线程 `worker()`：安装 `QtFiberScheduler` → `block_.wait()`（线程不退出、可调度协程）。`close()`：`block_.close()` 唤醒各线程，随后遍历 `tds_` 逐一 `join()`（带 `joinable()` 判断），阻塞到全部工作线程从 `worker()` 返回后才返回——保证工作线程停止先于 `FiberGlobalQueue` 等全局单例析构（理由见上文设计约束）。
...
**单元设计决策**：`FiberTask` 与 `SharedState` 分离——句柄可随意拷贝/移动传递，链上所有节点共享同一份状态；异常在协程边界统一转错误码（对应 DD-5）。`FiberApplication::quit()` 的收尾顺序固定（广播→排空（泵仍存活）→停泵→停池并 join→退出），确保协程、事件系统与工作线程在 Qt 应用对象及全局单例析构前完全收敛。顺序中两处约束不可调换：排空须在停泵之前（被唤醒协程要靠泵跑到终止），停池须 join 到工作线程退出（避免其在静态析构后访问 `FiberGlobalQueue`，见 §5.3）。
...
- **FiberApplication**：`installFiberApplication()`：主线程安装 `QtLocalFiberScheduler`，启动 `FibersPool`。`exec()`：主线程 `block.wait()`。`quit()`：① `QMetaObject::invokeMethod(qApp,"aboutToQuit",DirectConnection)` 广播关机，各 `Awaitable` 关闭 channel、唤醒挂起协程；② `drainUntilIdle()`（**泵仍存活**）：循环 `processEvents` + 派发 `DeferredDelete` + `this_fiber::yield`，直至就绪集合空闲，使被唤醒的协程跑到终止；③ `FiberScheduler::signalExit()` 置全局退出标志，令各线程常驻泵协程自行终止；④ `block.close()` + `FibersPool::close()`（**唤醒并 join 全部工作线程**）；⑤ `QCoreApplication::exit()`。
...
**单元设计决策**：`Awaitable` 内部共享 `FiberChannel`（生产者只捕获 channel，避免引用环），并由不透明 `AwaitableCloseGuard` 承载 `setOnClose` 注入的 RAII 断连；`coro()` 工厂按来源建立连接并注入 `bind_close`；`readAll` 建连时若已有缓冲数据立即投递（规避 check-then-wait 竞态）；`generate(a)` 以 shared_ptr 持有 `Awaitable` 适配调度器对 fiber 函数的拷贝。
...
| CSU_EXECUTOR | 详细设计 | RX_COROUTINE_SCHEDULING、RX_THREAD_BLOCK_INTERFACE、RX_QT_INTEGRATION（关机 join 析构顺序安全） | §5.3 |
...
| RX_QT_INTEGRATION | CSC_SCHEDULER、CSC_EXECUTOR、CSC_TASK / CSU_SCHEDULER、CSU_EXECUTOR、CSU_TASK / JK_TASK_APP、JK_SCHEDULER_ALGO / MS_SAFE_EXIT、MS_DYNAMIC_LIFECYCLE |
...
| RX_OTHER_QT_SHARED | CSU_SCHEDULER 约束（亲和语义）、CSU_EXECUTOR（§5.3 关机竞态）/ DD-2 |
