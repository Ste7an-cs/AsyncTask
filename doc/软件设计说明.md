...
**图例说明**：本时序展示 `Coro::quit()` 的四步安全退出，解决“协程活过 Qt 应用对象、退出期访问已销毁事件系统导致崩溃/卡死”的问题。**① 广播**：通过 `invokeMethod` 主动触发 `aboutToQuit`（框架不用 `QCoreApplication::exec()`，需主动发此信号），各 `Awaitable` 借此关闭自己的 channel。**② 唤醒收尾**：挂在 `await()` 的协程因 `pop` 得到关闭信号而返回、就地收尾。**③ 排空**：`drainUntilIdle()` 循环执行 `processEvents`、派发 `deleteLater`、`yield`，直到在途协程与事件跑净。**④ 停机**：关闭线程阻塞与线程池，最后 `QCoreApplication::exit()`。要点：**先让协程收敛、再拆事件系统**，四步次序不可颠倒。
...
- **退出**：`Coro::quit()`（SU-4）时序见 §5.2.5——广播 `aboutToQuit`→关闭各 channel→`drainUntilIdle` 排空→关线程池→`exit()`。
...
- `QtFiberScheduler : FiberScheduler`：覆写 `suspend_until`——无就绪协程时用 `QEventLoop::processEvents(AllEvents, 超时)` 泵一轮 Qt 事件再返回，使协程调度与 Qt 事件循环共存于同一线程；用于工作线程池。
...
- `quit()`：① `QMetaObject::invokeMethod(qApp,"aboutToQuit",DirectConnection)` 广播关机（各 Awaitable 借此关闭 channel）；② `drainUntilIdle()`：循环 `processEvents` + 派发 `DeferredDelete` + `this_fiber::yield`，直至就绪集合空闲；③ `block.close()` + `FibersPool::close()`；④ `QCoreApplication::exit()`。
...
| R6_BASIC_ABILITY Qt 集成与安全退出 | SU-2(QtFiberScheduler)、SU-3(QtFiberThread)、SU-4(FiberApplication) / IF-1,IF-4 / §5.2.5 |
