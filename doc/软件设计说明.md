...
> **待办：本图待重新导出。** 源文件 `img/sdd-05-csc_await.mmd` 已按 `ChannelHub` 新结构更新，但当前环境无 mermaid 工具链，svg 尚未重导，因此**上图仍显示改动前的结构**（`Awaitable` 直接持有 `AwaitableCloseGuard` 与单个 `ch_`）。以下方图例说明与 `.mmd` 源文件为准。重导命令：`npx @mermaid-js/mermaid-cli -i doc/img/sdd-05-csc_await.mmd -o doc/img/sdd-05-csc_await.svg`。
...
- **FiberChannel<T>**：字段 `boost::fibers::mutex mtx_`；`boost::fibers::condition_variable cv_`；`std::deque<T> q_`；`bool closed_`；`std::uint32_t capacity_`（队列容量上限，默认 `kDefaultCapacity=1024`，0 表示无限，见下）。`push(v)`：加锁；已关闭则拒绝；否则入队并 `cv_.notify_all()`（详见下）。`pop() -> Result<T>`：加锁，`cv_.wait` 直到队非空或已关闭；关闭且空→返回“无数据”错误；否则取队首并弹出。`pop_wait_for(timeout) -> Result<T>`：带超时版本，超时→返回“超时”错误。`close()`：置 `closed_` 并 `cv_.notify_all()`，唤醒全部等待者收敛。使用 fiber 版 mutex/cv，等待时**让出协程而非阻塞线程**。
  **`wait_peek(out)`/`wait_peek_for(out,timeout)`（非破坏性读取）**：等待条件与 `pop` 相同（队非空或已关闭），但取到队首后**只拷贝、不弹出**——元素仍留在队列里，可被反复读到同一个值，是"状态格"能反复读、`ChannelHub` 状态模式赖以实现的基础。终止语义与 `pop` **刻意不同**：`pop` 在"已关闭但队列仍有排队值"时会先把余量取完，`wait_peek` 只要 channel 已关闭就直接返回 `closed`，不论队列是否仍有值，`out` 也不写入。原因：peek 不消费，"取完余量"这个概念对它不成立；更关键的是，这是"状态"类用法唯一可能拿到的终止信号——若关闭后 `wait_peek` 仍返回成功，读取方将永远成功、永不报错，无从得知来源已经终止（这正是本次改动語义 3：拿"最后的值不可读"换"可检测的终止"）。
...
- **Awaitable<T>**：字段 `shared_ptr<ChannelHub<T>> hub_`（构造即 `make_shared`，一条数据流的分发端）；`shared_ptr<FiberChannel<T>> queue_`（自己独占的消费端队列）。默认构造新建 hub 与 queue 并 `hub_->attach(queue_)`；`explicit Awaitable(AwaitMode)` 以指定模式构造 hub（其余同默认构造）；订阅构造（供 `shared()` 使用）复用传入的 hub、新建自己的队列并 attach——因此自动继承 hub 的模式。析构 `hub_->detach(queue_.get())`。`void` 特化用 `ChannelHub<int>`/`FiberChannel<int>` 承载“发生一次”信号。`channel()` 返回 `hub_`（供生产者捕获）；`setOnClose(fn)` 透传给 `hub_->setOnClose(fn)`，注入的回调挂在 hub 内联的 `AwaitableCloseGuard` 上，在消费者归零（表中不再存在既存活又未关闭的队列）时于 hub 锁外恰好执行一次。
  ...
  **状态模式**（`hub_->isState()` 为真）：`await()`/`await_for(t)` 先查 `queue_->is_closed()`——自己这一路若已关闭（自己 `close()` 过，或整条流关闭时连带关掉了它），直接返回 `queue_->close_error()`，不去碰状态格（状态格不消费，没有"余量"概念，语义上没有别的选择）；否则读 `hub_->stateCell()->wait_peek(value)`/`wait_peek_for(value,t)`，成功即返回值，失败则返回状态格的 `close_error()`。两个方法都**不消费**：多次调用返回同一个值，直到 `resolve()` 写入新值或流关闭。
  `resolve(v)`=`hub_->push(v)`（状态模式下只写状态格，见下）；`close()`（默认 `no_message`）与 `close(error)` 都只作用于 `queue_`（关自己这一路），随后调用 `hub_->notifyClosed(error)` 触发归零判定——单消费者场景下自己关闭即是最后一路，行为与旧实现逐字一致。`isClosed()` 查 `queue_->is_closed()`，供 `Generator` 判断自己这一路是否收尾。`discardPending()`：队列模式下等价于 `queue_->discard_pending()`（只清自己这一路）；状态模式下改清 `hub_->stateCell()->discard_pending()`（清共享的状态格，此后所有消费者的 `await()` 都重新阻塞）——状态模式下不存在"自己独占的当前值"这个概念，这是结构决定的，无法只清自己那一份。按值 Awaitable 保持 move-only、跨线程安全；socket 方法改以 shared handle 返回。
...
- **ChannelHub<T> 与 Awaitable::shared()（`channelhub.hpp`）**：一条数据流的分发端。字段 `boost::fibers::mutex mtx_`；`std::vector<std::weak_ptr<FiberChannel<T>>> consumers_`（消费者队列表，弱引用，强引用只在各 `Awaitable` 手里）；`std::atomic_bool closed_`；`std::uint32_t capacity_`（默认 `FiberChannel<T>::kDefaultCapacity`，作为后续 `attach()` 挂载的默认容量）；`std::error_code close_error_`；`std::shared_ptr<FiberChannel<T>> state_`（状态格，见下）；`detail::AwaitableCloseGuard guard_`（内联的清理钩子，从旧版 `Awaitable` 迁入本文件）。`Awaitable::shared()` 语义为：新建一个 `Awaitable`，复用同一个 `hub_`、挂一条属于自己的新队列到 hub 的消费者表上——**平表**，源与订阅者在表上没有身份差别，`a->shared()->shared()` 只是同一张表上多一项，不再是链式的镜像树。

  **`AwaitMode` 与状态格**：`enum class AwaitMode { Queue, State }`，构造 `ChannelHub` 时确定、此后不可变（无 setter）。`Queue`（默认）即以上描述的分发行为。`State` 时构造函数额外 `state_ = make_shared<FiberChannel<T>>(); state_->setCapacity(1)`——一条容量恰为 1 的 `FiberChannel`，赋值即 `push`：容量 1 意味着新值自动挤掉旧值，天然就是"最新值"语义，读取则走 `state_->wait_peek()`（不消费，可反复读）。`state_` 是否非空即模式标志，`isState()` 判空即可，无需额外的枚举字段。队列模式下 `state_` 是空指针，不产生任何额外堆分配。
  状态模式下几个方法相应改道，`isState()`/`stateCell()` 是判断与取用入口：
  - `push(v)`：先判 `state_`，非空则只 `state_->push(std::move(v))` 并直接返回 `success`，**不进入 `consumers_` 遍历分支**——不向任何消费者队列扇出。内存占用因此与消费者数量、赋值次数都无关，恒为 O(1)（一条容量 1 的队列），这是相对队列模式"每个消费者一条最多 1024 深的队列"的核心优势。
  - `close(error)`：在置 `closed_` 之后，若 `state_` 非空则先 `state_->close(close_error_)`，再照常遍历 `consumers_` 逐条关闭——状态格与所有消费者队列一起被关闭，状态模式下"关闭"是唯一的终止信号（详见 `wait_peek` 的语义，§5.1）。
  - `discard_pending()`：若 `state_` 非空则先 `state_->discard_pending()` 清空状态格（容量 1 的队列清空即回到"无值"），再照常遍历清空每条消费者队列（消费者队列在状态模式下始终为空，因为 `push` 从不扇出到它们，这一步是为了与队列模式共用同一份实现、行为对称，不产生实际影响）。
  - `setCapacity(n)`：`state_` 非空时直接返回、忽略传入的 `n`——状态天然容量为 1，容量概念对它不适用，是显式的空操作而非静默失败。
  - `capacity()`：`state_` 非空时恒返回 `1`，不查 `capacity_` 字段。
  - `attach`/`detach`/`notifyClosed`/`markExhausted`（消费者归零判定、清理钩子触发）**不因模式而改变**：状态模式下 `consumers_` 表和空指针检查逻辑照常工作，因为每个 `Awaitable` 无论哪种模式都仍然 `attach` 了自己独占的 `queue_`（用于承接 `close()`/归零语义），只是这条队列在状态模式下永远收不到 `push` 投递的值——它此时只承担"自己这一路是否关闭"的记账职责，实际取值全部经 `stateCell()` 完成。
  ...
  **`close()` 不摘表，析构才摘**：`Awaitable::close(ec)` 只把自己那条队列标记关闭并调用 `hub_->notifyClosed(ec)`，**不**从 `consumers_` 里移除自己的槽位；真正的摘除只发生在 `Awaitable` 析构（`detach`）。这条约束由 `corotcpserver.hpp:135-137` 的用法逼出：消费者先 `close()` 掉流、再 `delete server`，`server` 析构时靠 `discard_pending()` 遍历 `consumers_` 清掉队列里即将随 server 一起悬空的 `QTcpSocket*`；若 `close()` 就把队列摘出表，`discard_pending()` 对它就成了静默空操作，消费者仍可能 pop 出已删除的野指针。`push()` 遍历时对已关闭的队列只跳过投递（省一次加锁与一次 `T` 拷贝）、不剔除，同样是为了让 `discard_pending()` 之后仍能触达它。
  ...
  **存储**：`FiberChannel<T>` 卸下镜像列表后 `sizeof` 由 168 回落到 160，本次改动新增的 `wait_peek`/`wait_peek_for` 不引入新字段，`sizeof(FiberChannel<T>)` 维持 160 不变。`sizeof(ChannelHub<T>)` 本次改动前为 160，新增 `state_`（一个 `shared_ptr<FiberChannel<T>>`）后**增至 176**（均 x86-64 / libstdc++ / glibc 实测）——该字段无论模式都占位，队列模式下恒为空指针、不产生堆分配，状态模式下额外 `make_shared` 出一条 160 字节的状态格 `FiberChannel`。单消费者场景下每个 `Awaitable` 仍是两次 `make_shared` 分配（旧版是 channel + 独立 guard，现在是 hub + queue），队列模式下总字节由 168+guard 增至 176+160=336；状态模式下再加一条状态格的 160，共 496；订阅越多越划算——旧版每个订阅者是“完整 channel 168 + 自己的 guard”，现在只是一条 160 的队列，hub（含状态格）共用。sizeof 数字由 `test/testfiberawait/tst_testfiberawait.cpp` 的 `test_case_channel_layout_size` 钉死，换平台需重测。
