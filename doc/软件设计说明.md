...
- **Awaitable/channel**：信号/future 等 `coro()` 工厂按值返回 move-only `Awaitable`；socket 包装器方法返回 `shared_ptr<Awaitable<T>>`。来源析构或 `aboutToQuit` 时关闭 channel，最后一个 Awaitable 或 shared handle 释放时经 `setOnClose` 断连。
...
**软件逻辑**：
- **Awaitable<T>**：字段 `shared_ptr<FiberChannel<T>> ch_`；`shared_ptr<void> guard_`（不透明生命周期守卫）。`void` 特化用 `FiberChannel<int>` 承载“发生一次”信号。`channel()`：返回 `ch_`。`setOnClose(fn)`：`guard_ = shared_ptr<void>(nullptr, [fn](void*){ if(fn) fn(); })`——最后一个持有者析构时执行 `fn`。`await()`=`ch_->pop()`；`await_for(t)`=`ch_->pop_wait_for(t)`；`resolve(v)`=`ch_->push(v)`；`close()` 正常关闭并在排空后给出 `no_message`，`close(error)` 保存第一个终止 error。按值 Awaitable 保持 move-only、跨线程安全；socket 方法改以 shared handle 返回。
