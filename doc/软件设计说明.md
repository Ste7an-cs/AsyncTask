...
- **FiberChannel<T>**：字段 `boost::fibers::mutex mtx_`；`boost::fibers::condition_variable cv_`；`std::deque<T> q_`；`bool closed_`；`std::uint32_t capacity_`（队列容量上限，默认 `kDefaultCapacity=1024`，0 表示无限，见下）。`push(v)`：加锁；已关闭则拒绝；否则入队并 `cv_.notify_one()`。`pop() -> Result<T>`：加锁，`cv_.wait` 直到队非空或已关闭；关闭且空→返回“无数据”错误；否则取队首。`pop_wait_for(timeout) -> Result<T>`：带超时版本，超时→返回“超时”错误。`close()`：置 `closed_` 并 `cv_.notify_all()`，唤醒全部等待者收敛。使用 fiber 版 mutex/cv，等待时**让出协程而非阻塞线程**。
  **容量与丢弃**：`capacity_` 紧跟 `closed_` 声明，复用其后原本存在的填充字节，不增加 `sizeof`（详见 §5.5 的存储讨论）。`push` 入队后若 `capacity_ != 0 && q_.size() > capacity_`，循环 `pop_front` 丢弃队首最旧值直至队列大小回落到上限，净大小停留在上限，不阻塞生产者也不让 `push` 返回失败；丢弃不改变 `closed_`，也不修改已保留的 `close_error_`。`setCapacity(n)` 在持锁状态下更新 `capacity_`；若新值小于当前排队数则立即从队首丢弃直至不超过新容量；`n=0` 取消上限，退回旧的无界行为。`setCapacity` 还会把新容量级联传播给 `mirrors_` 中每一个仍存活的镜像（递归调用镜像自身的 `setCapacity`），因此源上调用一次即可让所有订阅者（含 `a->shared()->shared()` 这类链式订阅）跟着改变上限，不必抢在订阅之前调用；`shared()` 新建订阅者时也会把源当前的容量原样赋给新 channel（均见 §5.5）。
...
- **广播镜像（`FiberChannel::mirrors_` 与 `Awaitable::shared()`）**：扇出机制放在 `FiberChannel::push` 而非 `Awaitable` 一层，因为生产者按约定只捕获 `channel()`、绝不持有 `Awaitable`（见 `corosocket.hpp:219` 的注释"业务槽只捕获 channel……绝不捕获 awaitable"）——凡是生产者能触达的地方就只有 channel，广播因而必须由 channel 自己实现，`Awaitable::shared()` 只是新建一个 `Awaitable` 并把其 channel 通过 `addMirror()` 挂到源 channel 上。镜像列表是 `std::unique_ptr<std::vector<std::weak_ptr<FiberChannel<T>>>> mirrors_`，未调用过 `shared()` 的 channel 上恒为空指针：`sizeof(FiberChannel<T>)` 因这一指针从 160 增至 168 字节，但 `make_shared<FiberChannel<T>>` 的实际请求字节数从 176 增至 184，测得 glibc 两者落入同一个 184 字节分配块，堆占用不变；`vector` 本身仅在首次 `addMirror()` 时才分配，从未 `shared()` 过的 channel 不产生这份堆分配，广播能力对不使用它的路径零成本。此后新增的容量上限字段 `capacity_`（`std::uint32_t`，见 §5.1）紧跟 `closed_` 声明，复用的正是 `mirrors_` 指针未能填满、`closed_` 之后本已存在的填充字节，`sizeof` 仍停在 168，没有新增开销——但这也是最后一次能"免费"塞入新字段的机会，下一次再加字段就会跨过 168 撑到 176。`test/testfiberawait/tst_testfiberawait.cpp:1199` 用 `QCOMPARE(sizeof(Coro::FiberChannel<int>), std::size_t(168))` 把这个边界钉死，一旦哪次改动使体积跳变会立刻被测试捕获。`push`、`close`、`discard_pending` 三处遍历 `mirrors_`（非空时）向每个仍存活的镜像转发同名操作，此三处即为同步扇出点。`close()` **不**清空 `mirrors_`：`discard_pending()` 依赖它在 `close()` 之后仍能触达镜像——`corotcpserver.hpp:135-137` 描述的场景是消费者先 `close()` 掉流、再 `delete server`，此时仍需经由镜像列表把即将随 server 一起悬空的 `QTcpSocket*` 从镜像队列中丢弃；若 `close()` 清空列表，`discard_pending()` 对镜像就成了静默空操作，订阅者可能弹出一个已删除的指针。`push()` 对 `weak_ptr` 已过期（订阅句柄已析构）的槽位做 swap-and-pop 剔除；但已关闭的镜像（订阅者关闭了自己的 `Awaitable` 但仍持有句柄）**只跳过投递、不剔除**——若像早前实现那样一并 erase，`discard_pending()` 就再也触达不到这条镜像，等 server 析构时它队列里的悬空指针就永远清不掉，重新引入了 `close()` 保留 `mirrors_` 所要防的 use-after-free；跳过投递仍省下了对已关闭镜像的一次加锁和一次 `T` 拷贝。`push()` 一旦发现源自身已关闭就提前返回、不再遍历镜像，`close()` 也不剔除失效槽位，因此 `discard_pending()` 是唯一还会遍历一个已关闭 channel 的镜像列表的扇出点，顺带用同样的 swap-and-pop 剔除失效槽位。**析构收敛**：`~FiberChannel()` 在源 channel 未经 `close()` 就消亡时，以 `connection_aborted` 关闭所有仍存活的镜像——不这样做的话镜像仍开着却再无生产者，订阅者的 `await` 会永久阻塞；用 `connection_aborted` 区分正常结束的 `no_message`，便于定位误丢源句柄。析构时引用计数已归零，不存在其他持有者，无需加锁；`FiberChannel` 本就因显式删除拷贝构造/赋值而不再隐式生成移动构造/赋值，新增析构不改变其可移动性。锁顺序恒为「源 → 镜像」：`push`/`close`/`discard_pending` 先持源 channel 的 `mtx_`，再调用镜像自身的同名方法（其内部再持镜像的 `mtx_`），从不反过来；`addMirror()` 保持 private 并只对 `Awaitable` 声明 friend，杜绝调用方把两个 channel 互相注册为镜像形成环。**已知限制**：订阅者队列与源 channel 一样是有容量上限的 FIFO（默认 1024，超限丢弃队首最旧值，`setCapacity` 会级联传播给存活的镜像，详见 §5.1）；对字节流而言丢弃是在流的**中间**开洞而非丢尾部，对承载 `QTcpSocket*`/`QNetworkDatagram` 等自身即代表资源的值而言则是丢失整个连接/数据报而非几个字节，需要按场景调大容量或传 0 取消上限，框架不提供丢弃计数一类的运行期信号，容量是唯一的调节手段；不做 replay，`shared()` 之后才产生的数据订阅者才可见；扇出在生产者调用 `push()` 的线程上同步完成，订阅者越多，每次 `push()` 的开销越大。
